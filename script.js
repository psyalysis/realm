...
const BASE_ANIMATION_SPEED = 2.5; // Tiles per second
const CAMERA_ARRIVAL_TIME = 1; // Camera should arrive at target within this time (seconds)
const TRANSITION_THRESHOLD = 0.15; // Start next move at 85% completion (0.15 of 1.0 tile remaining)
...
function initMultiplayer() {
    ...
    socket.on('playerHealthChanged', (data) => {
        ...
        if (data.id === myPlayerId) {
            ...
            updateHealthBar();
            // Note: updateHealthBar will trigger damage effect if health decreased
        } else {
    ...
    // Player collision - camera effect when we hit someone
    socket.on('playerCollision', (data) => {
        // Check if we're the attacker (we hit someone)
        if (data.attackerId === myPlayerId) {
            // Use position from event data (more reliable than otherPlayers map)
            if (data.targetX !== undefined && data.targetY !== undefined) {
                // Move camera to enemy and shake, then return to player
                triggerHitCameraEffect(data.targetX, data.targetY);
            } else {
                // Fallback: try to get from otherPlayers map
                const targetPlayer = otherPlayers.get(data.targetId);
                if (targetPlayer) {
                    triggerHitCameraEffect(targetPlayer.x, targetPlayer.y);
                }
            }
        }
    });
...
function centerCameraOnRedSquare() {
    ...
    // Center camera target on player
    gameState.cameraTarget.x = gameState.player.x - (tilesVisibleX / 2);
    gameState.cameraTarget.y = gameState.player.y - (tilesVisibleY / 2);
...
// Trigger camera effect when hitting an enemy
function triggerHitCameraEffect(enemyX, enemyY) {
    // Store original camera target
    const originalCameraX = gameState.cameraTarget.x;
    const originalCameraY = gameState.cameraTarget.y;
    
    // Move camera to enemy position
    moveCameraToPosition(enemyX, enemyY);
    
    // Add shake effect to viewport
    viewport.classList.add('damage-shake');
    
    // After 0.2 seconds, return camera to player and remove shake
    setTimeout(() => {
...
function updateAnimations() {
    ...
    // Camera always arrives within CAMERA_ARRIVAL_TIME seconds
    ...
    if (cameraDistance > 0.001) {
        // Calculate required speed to arrive within CAMERA_ARRIVAL_TIME seconds
        const requiredSpeed = cameraDistance / CAMERA_ARRIVAL_TIME;
...
function executeMovement(action) {
    ...
    if (isConnected && socket) {
        ...
        // Only optimistically update if the new position is valid
        if (newX >= 0 && newX < WORLD_WIDTH && 
            ...
            // Invalid move - don't update, wait for server rejection
            processNextMovement();
        }
        return;
    }
    ...
    if (moved) {
        ...
        // If movement was blocked (e.g., at boundary or wall), try next queued movement
        processNextMovement();
    }
}
...
function executeDash(direction) {
    ...
    if (isConnected && socket) {
        ...
        // Only update if final position is valid
...
function updateHealthDots(healthDotsContainer, health, maxHealth, playerLeft, playerTop, playerSize) {
    ...
    // Update each dot based on health (only if health changed)
    if (healthChanged) {
