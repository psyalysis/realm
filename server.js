...
function createPlayer(playerId) {
    ...
    const player = {
        ...
        isCastingMana: false,
        manaCastEndTime: 0,
        lastDamageTime: 0, // Track when player last took damage (for invincibility)
...
// Deal damage to a player (with invincibility cooldown)
function dealDamageToPlayer(targetPlayer, attackerId, damageAmount = 1) {
    const currentTime = Date.now() / 1000;
    const INVINCIBILITY_DURATION = 0.5; // 0.5 seconds of invincibility after taking damage
    
    // Check if player is still invincible from previous damage
    if (targetPlayer.lastDamageTime && (currentTime - targetPlayer.lastDamageTime) < INVINCIBILITY_DURATION) {
        return false; // Player is invincible, no damage dealt
    }
    
    // Deal damage
    targetPlayer.health = Math.max(0, targetPlayer.health - damageAmount);
    targetPlayer.lastDamageTime = currentTime;
...
function processMovement(playerId, action) {
    ...
    if (newX >= 0 && newX < WORLD_WIDTH && 
        ...
        if (victimPlayer) {
            ...
            const damageDealt = dealDamageToPlayer(victimPlayer, playerId, 1);
            if (damageDealt) {
                // Movement blocked, victim takes damage
                return { 
                    success: false, 
                    collision: true, 
                ...
                // Victim is invincible, block movement
                return { success: false, collision: true, invincible: true };
            }
        }
...
function processDash(playerId, direction) {
    ...
    if (hitPlayer) {
        ...
        const damageDealt = dealDamageToPlayer(hitPlayer, playerId, 1);
        if (damageDealt) {
            ...
            // Victim is invincible, dash stops before collision
            // Final validation: ensure position is not in a wall
            if (isWall(finalX, finalY)) {
                console.error(`CRITICAL: Dash invincible collision position is in wall at (${finalX}, ${finalY}), correcting...`);
                const validPos = findNearestValidPosition(finalX, finalY);
                finalX = validPos.x;
                finalY = validPos.y;
            }
            ...
            // Final check after setting position
            if (isWall(player.x, player.y)) {
                console.error(`CRITICAL: Player ${playerId} ended up in wall after dash invincible collision, correcting...`);
                const validPos = findNearestValidPosition(player.x, player.y);
                player.x = validPos.x;
                player.y = validPos.y;
                player.targetX = validPos.x;
            ...
            return { success: true, collision: true, invincible: true };
        }
    }
...
function getGameStateSnapshotDelta(excludePlayerId = null, lastSentState = new Map()) {
    ...
    for (const [id, player] of gameState.players.entries()) {
        ...
        if (hasChanged) {
            // Include cooldown times only if they're active (to reduce payload)
...
io.on('connection', (socket) => {
    ...
    socket.on('move', (action) => {
        ...
        if (result.success) {
            ...
            // Handle collision damage notification
